...
// HttpGet Get returns *HttpRequest with GET method.
...
func HttpPostJson(url string, json string) (statusCode int, body []byte) {
	jsonStr := []byte(json)
...
func HttpJsonPost(url string, jsonBody string, header map[string]string) []byte {
	req := HttpPost(url)
	req.Body(jsonBody)
