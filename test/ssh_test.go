...
func TestSSH(t *testing.T) {
	t.Run("praviteKeyTest", praviteKeyTest)
}

func praviteKeyTest(t *testing.T) {

	const port = "2021"
	newServer(port)

	callback, _ := sh.DefaultKnownHosts()
	...
	shClient := sh.NewSSHClient("127.0.0.1", port, config)
	res, err := shClient.Run("ls /home")
	fmt.Println(string(res), err)

}

func TestSSHPass(t *testing.T) {
	const port = "2022"
	newServer(port)
	config := sh.PasswordKey("melbahja", "123456", 20)
	shClient := sh.NewSSHClient("127.0.0.1", port, config)
